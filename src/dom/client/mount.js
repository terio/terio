...
export default function mount(node, parent) {
    ...
    const IS_HYDRATED = parent.childNodes.length === 1 &&
        parent.childNodes[0].hasAttribute &&
        parent.childNodes[0].hasAttribute(LOKI_ROOT);
    if(!IS_HYDRATED && parent.childNodes.length) {
        throw 'parent is not empty!';
    }
    if(IS_HYDRATED) {
        existingDOMNode = parent.childNodes[0];
        return createDOMNode(node, existingDOMNode, IS_HYDRATED, true);
    }
    return parent.appendChild(createDOMNode(node, existingDOMNode, IS_HYDRATED, true));
}
